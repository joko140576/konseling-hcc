...
// ─── LOAD DATA via JSONP (bypass CORS) ───────────────────
function loadData() {
  showState("loading");
  const btn = document.getElementById("btn-refresh");
  btn.classList.add("spinning");

  // Remove previous JSONP script tag if any
  ...
  const timer = setTimeout(() => {
    ...
    document.getElementById("db-error-msg").textContent =
      "Request timeout. Pastikan GAS_URL benar dan sudah di-deploy ulang dengan akses Anyone.";
    showState("error");
  }, 15000);

  // Global callback consumed by GAS response
  window.__hccCallback = function(json) {
    clearTimeout(timer);
    btn.classList.remove("spinning");
    try {
  ...
  script.src = GAS_URL + "?action=getData&callback=__hccCallback";
  script.onerror = () => {
    clearTimeout(timer);
    btn.classList.remove("spinning");
    document.getElementById("db-error-msg").textContent =
      "Gagal memuat. Cek GAS_URL di app.js dan pastikan deployment aktif.";
    showState("error");
  };
