...
function getAllData() {
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      const r = rows[i];
      // Build hasilKonseling from columns J onwards (index 9+)
      const hasilKonseling = {};
      topicKeys.forEach((key, ki) => {
        const couIdx      = 9 + ki * 2;
        const konselorIdx = 10 + ki * 2;
    ...
    return ContentService
      .createTextOutput(JSON.stringify({ status: "error", message: err.message, data: [] }))
      .setMimeType(ContentService.MimeType.JSON);
  }
}
...
function doPost(e) {
  try {
    // Support both JSON body and URL-encoded (no-cors workaround)
    let data;
